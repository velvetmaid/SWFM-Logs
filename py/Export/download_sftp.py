...
query_main = """
SELECT * FROM (
  SELECT DISTINCT ON (tpts.tx_site_id)
    tpts.tx_site_id, 
    tpts.pm_ticket_site_id, 
    tpspp.foto_pengukuran_kwh_bulan_sekarang_location 
  FROM 
    wfm_schema.tx_pm_site_power_pln tpspp 
  INNER JOIN 
    wfm_schema.tx_pm_ticket_site tpts 
    ON tpspp.tx_pm_ticket_site_id = tpts.pm_ticket_site_id
  WHERE 
    tpspp.foto_pengukuran_kwh_bulan_sekarang_location <> ''
    and tpts.pm_ticket_site_id like '%202407%'
  ORDER BY 
    tpts.tx_site_id, tpts.pm_ticket_site_id DESC
) AS subquery
ORDER BY pm_ticket_site_id desc
LIMIT 500;
"""
