WITH availabillity_ticket AS (
    ...
    FROM
        ...
        LEFT JOIN {tx_ticket_terr_opr} ttto ON ttto.pic_id = tumm.tx_user_mobile_management_id::varchar
...
list_role AS (
    ...
    FROM
        {tm_user_role} tur
        JOIN {mapping_user_mobile_role} mumr ON tur.tm_user_role_id = mumr.role_id
    GROUP BY
        mumr.tx_user_mobile_management_id
)
SELECT
...
FROM
    ...
    LEFT JOIN {tm_cluster} tc ON tumm.cluster_id :: bigint = tc.cluster_id
