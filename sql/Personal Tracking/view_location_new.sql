...
SELECT
    ...
    CASE
        WHEN EXISTS (
            ...
            SELECT
                string_agg(tx2.ticket_no :: text, ', ' :: text)
            FROM
                wfm_schema.tx_ticket_terr_opr tx2
            WHERE
                tx2.pic_id :: text = tx_user_mobile_management.tx_user_mobile_management_id :: character varying :: text
                AND tx2.status = 'IN PROGRESS' :: text
        )
        ELSE NULL :: text
...
FROM
    ...
    LEFT JOIN (
        ...
        FROM
            wfm_schema.tx_ticket_terr_opr tx_ticket_terr_opr_1
    ) tx_ticket_terr_opr ON tx_ticket_terr_opr.pic_id = tx_user_mobile_management.tx_user_mobile_management_id :: character varying
WHERE
...
SELECT
    ...
    CASE
        WHEN (
            ...
            WHERE
                tx_ticket_terr_opr.pic_id = tx_user_mobile_management.tx_user_mobile_management_id :: character varying
