...
SELECT
    ...
    tx_user_mobile_management.employe_name,
    tm_area.area_id,
    tm_area.area_name,
...
-- 
CREATE
OR REPLACE VIEW wfm_schema.vw_user_mobile_absent_location_x AS
SELECT
    tx_user_mobile_management.tx_user_mobile_management_id,
    ranked_absen.absentype,
    tx_location_device.currenttime,
    ...
    string_agg(
        DISTINCT CASE
            WHEN tx_ticket_terr_opr.status = 'IN PROGRESS' THEN tx_ticket_terr_opr.ticket_no
            ELSE NULL
        END,
        ', '
    ...
    CASE
        WHEN (
            EXISTS (
                ...
                WHERE
                    tx_ticket_terr_opr_1.pic_id = tx_user_mobile_management.tx_user_mobile_management_id :: character varying
                    AND tx_ticket_terr_opr_1.status = 'IN PROGRESS'
            )
        ) THEN 'Not Available'
        ELSE 'Available'
    END AS availability_status
FROM
    ...
    JOIN (
        ...
        WHERE
            tm_user_role_1.code = 'MUSERTS'
            OR tm_user_role_1.code = 'MUSERMBP'
    ) tm_user_role ON mapping_user_mobile_role.role_id = tm_user_role.tm_user_role_id
    RIGHT JOIN (
        SELECT
            ...
            tx_ticket_terr_opr_1.status
        FROM
            wfm_schema.tx_ticket_terr_opr tx_ticket_terr_opr_1
    ) tx_ticket_terr_opr ON tx_ticket_terr_opr.pic_id :: text = tx_user_mobile_management.tx_user_mobile_management_id :: text
WHERE
    ranked_absen.rank_amount < 2
GROUP BY
    tx_user_mobile_management.tx_user_mobile_management_id,
...
FROM
    ...
    JOIN (
        ...
        WHERE
            tm_user_role_1.code = 'MUSERTS'
            OR tm_user_role_1.code = 'MUSERMBP'
            AND tm_user_role_1.is_active = TRUE
    ) tm_user_role ON mapping_user_mobile_role.role_id = tm_user_role.tm_user_role_id
    LEFT JOIN (
        SELECT
            tx_ticket_terr_opr_1.tx_ticket_terr_opr_id,
        ...
        WHEN (
            EXISTS (
                ...
                FROM
                    wfm_schema.tx_ticket_terr_opr tx_ticket_terr_opr_1
                WHERE
                    tx_ticket_terr_opr_1.pic_id :: integer = tx_user_mobile_management.tx_user_mobile_management_id
                    AND tx_ticket_terr_opr_1.status = 'IN PROGRESS'
            )
        ) THEN string_agg(
            DISTINCT tx_ticket_terr_opr.ticket_no,
            ', '
        )
        ELSE NULL
    END AS tx_ticket_terr_opr_ticket_no,
...
FROM
    ...
    ) tm_nop ON tx_user_mobile_management.nop_id = tm_nop.nop_id
    LEFT JOIN (
        SELECT
            tm_cluster_1.cluster_id,
            tm_cluster_1.cluster_name
    ...
    LEFT JOIN (
        ...
        FROM
            wfm_schema.tx_ticket_terr_opr tx_ticket_terr_opr_1
    ) tx_ticket_terr_opr ON tx_ticket_terr_opr.pic_id :: int = tx_user_mobile_management.tx_user_mobile_management_id
WHERE
    ranked_absen.rank_amount < 2
GROUP BY
    tx_user_mobile_management.tx_user_mobile_management_id,
